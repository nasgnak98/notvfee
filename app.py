...
NOTE_MAP = {
    "기초수급": "기초수급자", "기초생활": "기초수급자", "수급자": "기초수급자",
    "시청각": "시청각장애", "장애인": "시청각장애", "장애": "시청각장애",
    "국가유공": "국가유공자", "유공자": "국가유공자"
}


def load_table(uploaded_file):
    """엑셀 파일에서 헤더 위치를 찾아 데이터를 로드합니다."""
    ...
    if skip_row is None:
        raise ValueError("⚠️ 파일에서 기준 헤더(세대번호)를 찾을 수 없습니다. 파일 양식을 확인해 주세요.")

    header_row = df_raw.iloc[skip_row]
    df = df_raw.iloc[skip_row + 1:].copy()
    df.columns = [str(val).strip() for val in header_row.values]

    key_col = next((c for c in df.columns if any(k in c for k in ["세대번호", "세대", "호수", "동호수"])), df.columns[0])
...
def extract_info(row, target_df):
    ...
    date_cols = [c for c in target_df.columns if any(k in str(c) for k in ["말소일", "면제일", "변동일", "등록일", "일자", "일시"])]
    for c in date_cols:
        val = row[c]
    ...
    reason_cols = [c for c in target_df.columns if any(k in str(c) for k in ["말소사항", "면제사항", "사유", "내용", "내역", "구분", "비고"])]
...
def generate_full_report(template_bytes, current_bytes, report_rows):
    ...
    for r in range(1, ws_tmpl.max_row + 1):
        ...
        if any(k in val for k in ["2. 변동사항", "2.변동사항", "3. 세대", "3.세대", "3. 미부과", "번호", "세대번호", "변동일"]):
            break
    ...
    for r in range(1, ws.max_row + 1):
        row_str = " ".join([str(ws.cell(row=r, column=c).value or "") for c in range(1, ws.max_column + 1)])
        if "주소" in row_str:
            address_row_idx = r
            break
    ...
    if report_rows:
        ...
        for idx, r_data in enumerate(report_rows, 1):
            ...
            for c_idx, val in enumerate(row_values, 1):
                c = ws.cell(row=insert_pos, column=c_idx, value=val if val is not None else "")
                c.font = FONT_BODY
                c.border = BORDER_ALL
                c.alignment = ALIGN_CENTER
...
with col1:
    tmpl_file = st.file_uploader("1️⃣ 안내사항 템플릿 (필수)", type=["xlsx", "xls"])
with col2:
...
if tmpl_file and file_a and file_b:
            ...
            with st.spinner("데이터 분석 및 보고서 결합 중..."):
                ...
                for k in deleted:
                    ...
                    if not sub_df.empty:
                        ...
                        dash_rows.append({
                            "구분": "🔴 삭제", "세대번호": k, "변동일": dt,
                            "내용": raw_reason if raw_reason != "-" else "부과 재개", "부과여부": "부과"
                        })
                        report_content, report_note = determine_report_text_and_note("삭제", raw_reason)
                        report_rows.append([None, k, dt, report_content, "부과", report_note])
                ...
                for k in added:
                    ...
                    if not sub_df.empty:
                        ...
                        dash_rows.append({
                            "구분": "🟢 신규", "세대번호": k, "변동일": dt,
                            "내용": raw_reason if raw_reason != "-" else "미부과 등록", "부과여부": "미부과"
                        })
                        report_content, report_note = determine_report_text_and_note("신규", raw_reason)
                        report_rows.append([None, k, dt, report_content, "미부과", report_note])

                excel_bytes = generate_full_report(tmpl_file.getvalue(), file_b.getvalue(), report_rows)

            st.subheader("📊 웹 대시보드 (변동사항 내역)")
            if dash_rows:
                df_dash = pd.DataFrame(dash_rows)
                st.dataframe(df_dash.style.set_properties(**{'text-align': 'center'}), use_container_width=True, hide_index=True)
            else:
                st.success("🎉 변동 사항이 없습니다.")

            st.divider()
            st.subheader("📥 최종 결과물 다운로드")
            st.success("🎉 보고서 생성이 완료되었습니다! 아래 다운로드 버튼을 클릭해 주세요.")
    ...
    st.info("🚨 3개 파일(1️⃣ 안내사항 템플릿, 2️⃣ 과거 파일, 3️⃣ 현재 파일)을 모두 업로드해 주세요.")
